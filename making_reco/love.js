...
scan=(rnXY,f,d=0.002)=>{
  ...
  for(;y<y1;y+=d)for(x=x0;x<x1;x+=d)if(Math.abs(f(x,y))<sil) g.fillRect((x-x0)*kx,N-(y-y0)*ky, 1,1);
}
scan([-3,3, -5, 1],love(2.23))

//给你的爱心上色<3
e.onclick=ev=>{ g.fillStyle="red"; bfsClosen(g,ev.offsetX,ev.offsetY, (x,y)=>{g.fillRect(x,y,1,1)})  }
bfsClosen=(g,x0,y0,f_xy,test=(i,a)=>a[i+3]==0)=>{ //RGBA, i+3:透明
  let a=g.getImageData(0,0,N,N),w=a.width,
   pd=[-1,1,-w,w], ps=[y0*w+x0],pNo=new Set;
  a=a.data;
  if(!test(ps[0]*4,a))return;//才源点,不符合.
  let p0,d,p1;
  while((p0=ps.shift()) !=null){
    for(d of pd){p1=p0+d; if(!test(p1*4,a)|pNo.has(p1))continue; f_xy(p1%w, p1/w); ps.push(p1);pNo.add(p1)}
  }
}
