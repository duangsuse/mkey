...
const next=g=>g.next().value,
  nextN=(n,g)=>{
    if(!isStrm(g))throw g;  if(!g.next)g=g();//g=pull(g)
  ...
  pull=f=>(r=>!isFunc(r)?r: pull(r))(f()), //=trampoline, for auto iterable->iter
  isStrm=o=>o.next||isFunc(o),
  map2D=(op,a)=>a.map(r=>r.map(op)),

  useEquiv=([f,back], get_op)=>(r=>!r.n_st1? r.vals.map(back) : map2D(back,r.vals) )(st.go(get_op(f), equ.nGo)) // checked: dup of vals()
{
  const
...
class State{
  ...
  get(v){
    ...
    let r=this.d.get(v); return (r!==undef)? grab(this,r)||r : grab(this,v)||v // null is value. // checked: just (r=undef)?r: 
  }
  ...
  static deepEq=(a,b)=>{
    let {isEq,cfg}=State;
    if(a instanceof Array){ if(n(a)!=n(b))return false; for(let i=0,N=n(a);i<N;i++)if(!isEq(a[i], b[i]))return false; } // checked: dup of canUnify&deepEq(^3)&unify
    else{
  ...
  static deepCopy=(s0,o)=>{
    ...
    else{ o1=Object.create(o.__proto__); State.cfg.keys(o).forEach(k=>{ o1[k]=s0.get(o[k]); }) } // {...[[k,v]].map} is also usable
    return o1
  }
...
if(`support product type like [a,b]`)Object.assign(State, {
  ...
  isEq(a,b){
    return (State.canUnify(a,b)&&State.deepEq(a,b)) ||a===b // checked: dup of unify:keys reduce
  },
  grab(s0,v){
